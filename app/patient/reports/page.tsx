...
export default function ReportsPage() {
  ...
  useEffect(() => {
    if (user) {
      const fetchReports = async () => {
        try {
          setIsLoading(true);
          // Busca todos os laudos vinculados a este paciente
          const fetchedReports = await reportsApi.getReports(user.id);

          // Ordena pelo mais recente
          fetchedReports.sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  ...
  const handleDownloadReport = async (reportId: string) => {
    ...
    try {
      ...
      link.href = url
      link.download = `laudo-${report.order_number || 'exame'}.html`
  ...
  return (
    <Sidebar>
      <div className="space-y-6">
        ...
        {/* Removido os cards de "Pendentes" e "Disponíveis", mantendo apenas o Total ou simplificando */}
        <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total de Laudos Disponíveis</CardTitle>
        ...
        {reports.length > 0 && (
          <div>
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Lista de Laudos</h2>
            <div className="grid gap-4">
              {/* Mapeia TODOS os reports sem filtrar status */}
              {reports.map((report) => (
                <Card key={report.id} className="hover:shadow-md transition-shadow border-l-4 border-l-blue-500">
                      ...
                      <div className="space-y-1">
                        ...
                        <CardDescription className="flex items-center gap-4">
                          <span className="flex items-center gap-1">
                            ...
                            {report.requested_by || "Médico Responsável"}
                          </span>
                          <span className="flex items-center gap-1">
                  ...
                  <CardContent>
                    <p className="text-gray-600 mb-4 line-clamp-2">{report.diagnosis}</p>
        ...
        {reports.length === 0 && !isLoading && (
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-gray-600">Seus laudos médicos aparecerão aqui assim que forem emitidos pelo médico.</p>
            </CardContent>
          </Card>
        )}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <DialogDescription className="mt-1">
                    Pedido: {selectedReport?.order_number}
                  </DialogDescription>
                </div>
